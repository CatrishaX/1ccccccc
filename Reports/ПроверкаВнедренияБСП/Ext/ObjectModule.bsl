	...
	Для Каждого РеквизитВРоли Из ТаблицаРеквизитовВРоли Цикл
		...
		Если Не ЭтоОбъектБСП(ОбъектМетаданных) И ЭтоПоле Тогда
			...
			Если (УстанавливатьПраваДляНовыхОбъектов
				...
				И ПравоДоступа("Чтение", ОбъектМетаданных, Метаданные.Роли[ИмяРоли]))
		...
		Если ПраваНаПоле["View"] <> Истина Тогда
			...
			КонецЕсли;
			ТекстОшибки = ?(Исправлять, НСтр("ru = 'Исправлено.'"), "") 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" без права Просмотр.'"), ИмяРеквизита);
